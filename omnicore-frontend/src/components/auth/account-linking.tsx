"use client";

import { useState } from "react";
import { useAuth } from "@/contexts/AuthContext";
import { Button } from "@/components/ui/button";
import { linkWithPopup } from "firebase/auth";
import { googleProvider } from "../../../firebase";

interface AccountLinkingProps {
  onSuccess?: () => void;
  onError?: (error: string) => void;
}

export function AccountLinking({ onSuccess, onError }: AccountLinkingProps) {
  const { currentUser } = useAuth();
  const [loading, setLoading] = useState(false);
  // Check if Google provider is already linked
  const hasGoogleProvider = currentUser?.providerData?.some(
    (provider) => provider.providerId === "google.com"
  );

  async function linkGoogleAccount() {
    if (!currentUser) return;

    try {
      setLoading(true);
      // Link the Google account to the current user
      await linkWithPopup(currentUser, googleProvider);
      if (onSuccess) onSuccess();
    } catch (error) {
      console.error("Error linking Google account:", error);
      if (onError) {
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Failed to link Google account";
        onError(errorMessage);
      }
    } finally {
      setLoading(false);
    }
  }
  if (hasGoogleProvider) {
    return (
      <div className="flex items-center space-x-2 mt-4">
        <div className="flex items-center bg-accent/20 text-accent-foreground px-3 py-1 rounded-md text-sm">
          <svg className="w-4 h-4 mr-1" viewBox="0 0 24 24">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
            <path d="M1 1h22v22H1z" fill="none" />
          </svg>
          Google Account Linked
        </div>
      </div>
    );
  }

  return (
    <div className="mt-4">
      <h3 className="text-sm font-medium mb-2">Link additional accounts</h3>
      <Button
        onClick={linkGoogleAccount}
        disabled={loading}
        variant="outline"
        className="flex items-center justify-center gap-2"
      >
        {loading ? (
          <div className="h-4 w-4 animate-spin rounded-full border-b-2 border-primary"></div>
        ) : (
          <svg className="w-4 h-4" viewBox="0 0 24 24">
            <path
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              fill="#4285F4"
            />
            <path
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              fill="#34A853"
            />
            <path
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              fill="#FBBC05"
            />
            <path
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              fill="#EA4335"
            />
            <path d="M1 1h22v22H1z" fill="none" />
          </svg>
        )}
        {loading ? "Linking..." : "Link Google Account"}
      </Button>
      <p className="mt-2 text-xs text-muted-foreground">
        Linking your Google account provides an alternative way to sign in and
        enhances account security.
      </p>
    </div>
  );
}
