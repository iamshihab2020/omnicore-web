...
class MenuItemAddonGroup(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    tenant = models.ForeignKey(Tenant, on_delete=models.CASCADE, related_name='addon_groups')
    name = models.CharField(max_length=50)
...
class MenuItemAddonGroupItem(models.Model):
    ...
    """
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    group = models.ForeignKey(MenuItemAddonGroup, on_delete=models.CASCADE, related_name='items')
    addon = models.ForeignKey(MenuItemAddon, on_delete=models.CASCADE, related_name='groups')
    display_order = models.IntegerField(default=0)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
